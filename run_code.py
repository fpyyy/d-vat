...
def make_env(
    ...
    render: bool = False,
    ts: float = 0.1,
    observation_buffer_length: int = 3,
) -> Callable:
    def _init() -> gym.Env:
        env = UnrealTrackingEnv(
            rank,
            test=test,
            render=render,
            ts=ts,
...
if __name__ == "__main__":
    ...
    if settings["eval_mode"]:
        ...
        model = SAC.load(os.path.join(f"experiments/SAC_{1711119972}", f"SAC_{0}"), env=eval_env)

        # Eval
        mean_reward, std_reward = evaluate_policy(
            model, eval_env, n_eval_episodes=settings["eval_episodes"]
        )
        ...
        current_path = Path(os.path.join(abs_path, "experiments", f"SAC_{t}"))
        ...
        while True:
            ...
            if mean_reward > best_episodes[worst_model]:
                ...
                model.save(os.path.join(current_path, f"SAC_{worst_model}"))
